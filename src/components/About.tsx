...
export const About:React.FC<ComponentProps> = ({lenguage}) => {
    ...
    className="flex flex-col xl:h-[120vh] 2xl:h-[80vh] md:mt-72 lg:mt-72 xl:mt-0 gap-24 lg:gap-64 justify-center items-center">
        <h3 className="text-3xl lg:text-2xl xl:text-5xl font-medium">{lenguage === "EN" ? "About me" : "Sobre mi"}</h3>
        <div className="flex flex-col lg:w-1/3 border-gray-500 border-2 p-8 rounded-lg gap-20 shadow-lg">
            <p className="text:lg lg:text-2xl">Busco adentrarme en el mundo IT como desarrollador frontend, soy una persona motivada a crecer, aprender y
...
Tengo 24 años y vivo en Tigre, Buenos Aires.</p>
        <a href="https://filetools13.pdf24.org/client.php?mode=inline&file=joinPdf_675b3b8b16774_1589679650179108536.pdf&action=getFile" target="_blank" className="self-center mb-8">
            <Button variant="outlined" color="inherit" size="large" className="w-38 lg:w-48 scale-75 lg:scale-100 hover:bg-slate-700 self-center">Curriculum</Button>
            </a>
        </div>
    </motion.section>
    </AnimatePresence>
