...
export const Main: React.FC<ComponentProps> = ({lenguage}) => {
    return (
        <>
        <main id="home" className="w-screen flex flex-col lg:flex-row sm:min-h-[90vh] lg:overflow-hidden lg:justify-normal gap-10 lg:gap-0 justify-center items-center">
            <div className="w-1/2 flex justify-center items-center">
                <motion.img
                initial ={{ opacity: 0,translateX: -50, rotate: -5 }}
                animate ={{ opacity: 1,translateX: 0, rotate: 0 }}
                transition={{ duration: 1.5 }}
                className="filter brightness-115 contrast-115 saturate-110 w-2/5 rounded-2xl min-h-56 min-w-56 lg:min-h-0 lg:min-w-0 center" src="/foto.png"></motion.img>
            </div>
            ...
            className="lg:w-2/5 flex flex-col justify-center gap-3 lg:gap-7 self-start ml-7 lg:self-center">
                <h1 className="text-2xl xl:text-4xl font-bold">{lenguage === "EN" ? "Hi, I'm Ulises Molina" : "Hola, soy Ulises Molina"}</h1>
                <h2 className="xl:text-xl  font-normal">{lenguage === "EN" ? "I'm a Frontend developer from Buenos Aires, Argentina" : "Desarrollador frontend de Buenos Aires, Argentina"}</h2>
                <p className="xl:text-lg">{lenguage === "EN" ? "I like to be in constant growth with the ability to learn new technologies and adapt easily to new work teams. Passionate about challenges and even more so  if they enrich my knowledge. Completely self-taught." : "Me gusta estar en constante crecimiento con capacidad de aprender nuevas tecnologías y adaptarme fácilmente a nuevos equipos de trabajo. Apasionado por los retos y más si enriquecen mis conocimientos. Completamente autodidacta."}</p>
                <div className="flex flex-col gap-10 mt-10 ">
                    <p className="xl:text-xl">{lenguage === "EN" ? "Principal technologies" : "Tecnologías principales"}</p>
                ...
                <div className="flex flex-col gap-10 mt-10 ">
                    <p className="xl:text-xl">{lenguage === "EN" ? "Tools and others" : "Herramientas y otros"}</p>
                    <div className="flex gap-5 lg:gap-20">
